...
static int huawei_nt51021_prepare(struct drm_panel *panel)
{
	...
	gpiod_set_value_cansleep(ctx->power_blk_gpio, 1);   /* disp_power_blk */
...
static int huawei_nt51021_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx->bl_gpio = devm_gpiod_get(dev, "vled", GPIOD_OUT_LOW);
	if (IS_ERR(ctx->bl_gpio))
		return dev_err_probe(dev, PTR_ERR(ctx->bl_gpio), "Failed to get vled-gpios\n");
